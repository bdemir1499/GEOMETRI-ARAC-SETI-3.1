// =================================================================
// KESİN SES SUSTURUCU (NUCLEAR MUTE) - EN ÜSTTE OLMALI
// =================================================================
window.Audio = class {
    constructor() {
...
// Mevcut tüm ses nesnelerini geçersiz kıl
const muteAll = () => {
    const silent = { play: () => Promise.resolve(), pause: () => {}, stop: () => {}, volume: 0 };
    window.audio_click = silent;
    window.audio_undo = silent;
    window.audio_draw = silent;
    window.audio_eraser = silent;
    window.audio_clear = silent;
};
muteAll();
// =================================================================


// --- SESLER (TAMAMEN SUSTURULDU) ---
const silentAudio = { 
...
window.audio_clear = silentAudio; // Eğer varsa bu da sussun

let audio_click_src_set = true; // Hata vermemesi için true kalsın
...
document.querySelectorAll('#polygon-options .tool-button-sub').forEach(buton => {
    buton.addEventListener('click', () => {
        document.getElementById('polygon-options').classList.add('hidden');
    });
});
            ...
            // B) RESİM (IMAGE)
                ...
                if (stroke.img) {
                    ...
                    if (selectedItem === stroke) {
                        ...
                        ctx.fill();
                        ctx.stroke();
// ▼▼▼ YENİ: MAVİ DÖNDÜRME KULBU ▼▼▼
                        const rotY = -stroke.height / 2 - 30; 

                        // Bağlantı çizgisi
                        ctx.beginPath();
                        ctx.moveTo(0, -stroke.height / 2);
                        ctx.lineTo(0, rotY);
...
// J) DİKDÖRTGEN (RECTANGLE)
                ...
                ctx.strokeStyle = stroke.color || '#00ffcc';
                ctx.lineWidth = stroke.width_line || 3;
                ctx.stroke();
                
                // Seçiliyken Tutamaçları Çiz
                if (selectedItem === stroke && currentTool === 'move') {
                    // Pembe (Boyutlandırma) Tutamacı
                    ctx.beginPath();
                    ctx.arc(stroke.width / 2, stroke.height / 2, 10, 0, 2 * Math.PI);
                    ctx.fillStyle = '#FF00FF'; ctx.fill(); ctx.stroke();
                    
                    // Mavi (Döndürme) Tutamacı
                    const rotY = -stroke.height / 2 - 30;
                    ctx.beginPath(); ctx.moveTo(0, -stroke.height / 2); ctx.lineTo(0, rotY); ctx.stroke();
                    ctx.beginPath(); ctx.arc(0, rotY, 12, 0, 2 * Math.PI);
                    ctx.fillStyle = '#2196F3'; ctx.fill(); ctx.stroke();
                }
                ctx.restore();
            }
            ...
            // I) ÇEMBER / YAY (ARC)
                ...
                // ▼▼▼ ÇEMBER BİLGİSİ (Pİ = 3 VE ADIM ADIM FORMÜL) ▼▼▼
                if (stroke.showCircleInfo) {
                    const r_px = stroke.radius;
                    const pixelPerCm = 30; 
                    const r_cm = (r_px / pixelPerCm).toFixed(1);
                    
                    const cevre = (2 * 3 * parseFloat(r_cm)).toFixed(1);
                    const alan = (3 * Math.pow(parseFloat(r_cm), 2)).toFixed(1);

                    const labelX = stroke.cx + r_px + 15;
                    let labelY = stroke.cy - 25;

                    ctx.save();
                    ctx.fillStyle = '#00ffcc'; // Neon Yeşil
                    ctx.font = 'bold 15px Arial';
                    ctx.textAlign = 'left';

                    ctx.fillText(`r = ${r_cm} cm`, labelX, labelY);
                    labelY += 25;
                    ctx.fillText(`Çevre = 2 . π . r`, labelX, labelY);
                    labelY += 20;
                    ctx.fillText(`= 2 . 3 . ${r_cm} = ${cevre} cm`, labelX, labelY);
                    labelY += 30;
                    ctx.fillText(`Alan = π . r²`, labelX, labelY);
                    labelY += 20;
                    ctx.fillText(`= 3 . (${r_cm})² = ${alan} cm²`, labelX, labelY);
                    labelY += 25;
                    ctx.font = 'italic 13px Arial';
                    ctx.fillStyle = '#aaaaaa';
                    ctx.fillText(`(π = 3 alındı)`, labelX, labelY);
                    ctx.restore();

                    // Yarıçap Çizgisi (Pembe Kesik Çizgi)
                    ctx.save();
                    ctx.beginPath();
                    ctx.moveTo(stroke.cx, stroke.cy);
                    ctx.lineTo(stroke.cx + r_px, stroke.cy);
...
// DİKDÖRTGEN BUTONU TIKLANMA OLAYI
...
// --- app.js: Canlandır Butonu (SADECE MOD DEĞİŞTİRİR) ---
if (animateButton) {
    const toggleAnimate = (e) => {
        // Dokunmatik ekranlarda çift tetiklenmeyi önle
        if (e && e.cancelable) e.preventDefault(); 
        if (e) e.stopPropagation(); 

        // Modu Değiştir (Aç / Kapat)
        setActiveTool(currentTool === 'snapshot' ? 'none' : 'snapshot');
        
        // Araç değiştiğinde eski seçimleri temizle
        selectedItem = null;
...
// =====================================================================
// V3 + V2 SÜPER MOTOR (PARDUS, TABLET, PC, 3D VE DİKDÖRTGEN DESTEKLİ)
// =====================================================================

// --- POINTER TAKİP SİSTEMİ ---
...
// --- GRAFİK TABLET SİMÜLATÖRÜ VE BASINÇ OKUYUCU ---
function getPointerInfo(e) {
    const testModuAcik = false; // PC'de denemek istersen true yap
    if (testModuAcik && e.pointerType === 'mouse') {
        return { type: 'pen', pressure: Math.random() * 0.8 + 0.2 };
    }
    return { type: e.pointerType, pressure: e.pressure || 1 };
}

// --- KURŞUN GEÇİRMEZ KOORDİNAT OKUYUCU ---
...
// =====================================================================
// 1. POINTER DOWN (DOKUNMA/TIKLAMA BAŞLANGICI)
// =====================================================================
canvas.addEventListener('pointerdown', (e) => {
    if (e.cancelable) e.preventDefault(); // Pardus Kayma Koruması

    // --- HAYALET PARMAK SIFIRLAYICI ---
    if (e.pointerType === 'touch' && e.touches && e.touches.length === 1) {
        pointers.clear(); lastDist = 0;
    }
    ...
    // --- 3D ETKİLEŞİM KÖPRÜSÜ ---
    const isDrawingTool = ['pen', 'eraser', 'point', 'straightLine', 'line', 'segment', 'ray', 'draw_rectangle', 'draw_circle'].includes(currentTool) || currentTool.startsWith('draw_polygon_');
    
    if (!isDrawingTool && window.Scene3D && window.Scene3D.isInit) {
        const is3DHit = window.Scene3D.onDown(pos.x, pos.y);
        if (is3DHit) {
            window.isMoving3D = true; // 3D hareket başladı
            return; 
        }
    }

    // --- SİLGİ İŞLEMİ (TIKLANAN YERİ SİL) ---
    if (currentTool === 'eraser') {
        isDrawing = true;
        if (typeof handleEraser === 'function') handleEraser(pos);
        else if (typeof handleEraseEvent === 'function') handleEraseEvent(e);
        return;
    }

    // --- 2D TAŞIMA VE DİĞER ÇİZİM ARAÇLARI BAŞLATMA ---
    if (currentTool === 'none') return;
    
    // (Mevcut 'switch(currentTool)' mantığın burada başlayacak. 
    // Kalem, çizgi, çokgen ve dikdörtgen başlatma işlemleri V2'deki gibi aynı kalıyor.)
    if (currentTool === 'pen') {
        isDrawing = true; 
        ...
        drawnStrokes.push({ 
            ...
            baseWidth: currentPenWidth // V3 KALEM KALINLIĞI BAĞLANDI
        });
    } else if (currentTool === 'draw_rectangle') {
        isDrawingRectangle = true; 
        rectStartPoint = pos; 
    }

else if (currentTool === 'draw_circle' || (currentTool && currentTool.startsWith('draw_polygon_'))) {
        isDrawingPolygon = true; // Çokgen/Çember çizim modu aktif
        rectStartPoint = pos; // Merkez noktasını kaydet
    }

    // ... Diğer çizim araçlarının başlangıçları (nokta, çizgi vb.)
}, { passive: false });

// =====================================================================
// 2. POINTER MOVE (SÜRÜKLEME VE ZOOM)
// =====================================================================
canvas.addEventListener('pointermove', (e) => {
    ...
    // --- PARDUS HAYALET FARE ENGELLEYİCİ ---
    ...
    // --- İKİ PARMAK ZOOM (PDF/RESİM VE 3D) ---
    if (pointers.size > 1 && e.isPrimary === false) return; // Güvenlik yaması
    ...
    if (pointers.size === 2 || (e.touches && e.touches.length === 2)) {
        ...
        // A) 3D ZOOM
        if (window.Scene3D && window.Scene3D.currentMesh && window.Scene3D.handlePinch) {
            if (!window.Scene3D.initialPinchDistance) window.Scene3D.startPinch(currentDist);
            window.Scene3D.handlePinch(currentDist);
            return;
        }
    ...
    // --- 3D SÜRÜKLEME ---
    if (window.isMoving3D && window.Scene3D) {
    ...
    if (isDrawing && currentTool === 'pen') {
        if (drawnStrokes.length > 0) {
            ...
            currentStroke.path.push({x: pos.x, y: pos.y, p: pressureMove});
            redrawAllStrokes(); 
        }
        return;
    } 
    ...
    // K) DİKDÖRTGEN ÖNİZLEME VE GERÇEK ZAMANLI ÖLÇÜM
    else if (isDrawingRectangle && currentTool === 'draw_rectangle' && rectStartPoint) {
        redrawAllStrokes(); 
        
        const rw = Math.abs(pos.x - rectStartPoint.x);
        const rh = Math.abs(pos.y - rectStartPoint.y);
        
        const rx = Math.min(rectStartPoint.x, pos.x) + rw / 2;
        const ry = Math.min(rectStartPoint.y, pos.y) + rh / 2;

        ctx.save();
        ctx.beginPath();
        ctx.rect(Math.min(rectStartPoint.x, pos.x), Math.min(rectStartPoint.y, pos.y), rw, rh);
        
        ctx.strokeStyle = typeof currentLineColor !== 'undefined' ? currentLineColor : '#00ffcc';
        ctx.lineWidth = 3;
        ctx.setLineDash([5, 5]); 
        ctx.stroke();

        ctx.setLineDash([]); 
        ctx.fillStyle = typeof currentLineColor !== 'undefined' ? currentLineColor : '#00ffcc';
        ctx.font = "bold 16px Arial";
        
        ctx.textAlign = "center";
        ctx.textBaseline = "bottom";
        ctx.fillText(Math.round(rw) + " br", rx, Math.min(rectStartPoint.y, pos.y) - 8);
        
        ctx.textAlign = "left";
        ctx.textBaseline = "middle";
        ctx.fillText(Math.round(rh) + " br", Math.max(rectStartPoint.x, pos.x) + 8, ry);
        
        ctx.restore();
    }

    // L) ÇEMBER VE ÇOKGEN ÖNİZLEME
   // BÖYLE DEĞİŞTİR:
else if (isDrawingPolygon && currentTool && (currentTool === 'draw_circle' || currentTool.startsWith('draw_polygon_')) && rectStartPoint) {
        redrawAllStrokes();
        const r = Math.hypot(pos.x - rectStartPoint.x, pos.y - rectStartPoint.y);
        ctx.save();
        ctx.beginPath();
        
        if (currentTool === 'draw_polygon_circle') { // ÇEMBER İSMİ DÜZELTİLDİ
            ctx.arc(rectStartPoint.x, rectStartPoint.y, r, 0, 2 * Math.PI);
        } else {
            const sides = parseInt(currentTool.split('_')[2]) || 3; 
            for (let i = 0; i < sides; i++) {
                const angle = i * (2 * Math.PI / sides) - Math.PI / 2;
                const px = rectStartPoint.x + r * Math.cos(angle);
                const py = rectStartPoint.y + r * Math.sin(angle);
        ...
        ctx.fillStyle = typeof currentLineColor !== 'undefined' ? currentLineColor : '#00ffcc';
        ctx.font = "bold 16px Arial";
        ctx.fillText("r: " + Math.round(r) + " br", pos.x + 10, pos.y + 10);
        ctx.restore();
    }
    
    // ... Diğer taşıma (Move) ve önizleme kodları varsa buraya gelebilir
}, { passive: false });


// =====================================================================
// 3. POINTER UP (DOKUNMA/TIKLAMA BİTİŞİ)
// =====================================================================
window.addEventListener('pointerup', (e) => {
    if (e.pointerType === 'touch' && e.cancelable) e.preventDefault();

    // --- HAYALET FARE KALKIŞ ENGELLEYİCİ ---
    if (e.pointerType === 'mouse') {
        let hasTouch = false;
        for (let p of pointers.values()) {
            if (p.pointerType === 'touch' || p.pointerType === 'pen') hasTouch = true;
    ...
    // 1) DİKDÖRTGEN KAYDETME
    if (currentTool === 'draw_rectangle' && isDrawingRectangle && rectStartPoint && finalPos) {
        const rw = Math.abs(finalPos.x - rectStartPoint.x);
        const rh = Math.abs(finalPos.y - rectStartPoint.y);
        if (rw > 5 && rh > 5) { // Sadece tıklandıysa çizme, sürüklendiyse çiz
            const rx = Math.min(rectStartPoint.x, finalPos.x) + rw/2;
            const ry = Math.min(rectStartPoint.y, finalPos.y) + rh/2;
            drawnStrokes.push({ type: 'rectangle', x: rx, y: ry, width: rw, height: rh, color: typeof currentLineColor !== 'undefined' ? currentLineColor : '#00ffcc', width_line: 3 });
        }
    ...
    // 2) ÇEMBER VE ÇOKGEN KAYDETME
    else if (isDrawingPolygon && currentTool && currentTool.startsWith('draw_polygon_') && rectStartPoint && finalPos) {
        const r = Math.hypot(finalPos.x - rectStartPoint.x, finalPos.y - rectStartPoint.y);
        
        if (r > 5) {
            if (currentTool === 'draw_polygon_circle') {
                // Senin fonksiyonun bunu 'arc' olarak ve cx, cy ile tanıyor
                drawnStrokes.push({ 
                    type: 'arc', 
                    cx: rectStartPoint.x, 
                    cy: rectStartPoint.y, 
                    ...
                    startAngle: 0,
                    endAngle: 360,
                    color: typeof currentLineColor !== 'undefined' ? currentLineColor : '#00ffcc', 
                    width: 3,
                    fillColor: null,
                    showCircleInfo: false
                });
            } else {
                // Çokgenleri de center: {x,y} ve sideCount olarak tanıyor
                const sides = parseInt(currentTool.split('_')[2]) || 3;
                drawnStrokes.push({ 
                    type: 'polygon', 
                    center: { x: rectStartPoint.x, y: rectStartPoint.y }, 
                    radius: r, 
                    sideCount: sides, 
                    rotation: 0,
                    color: typeof currentLineColor !== 'undefined' ? currentLineColor : '#00ffcc', 
    ...
    // ... Eğer çizgi (line, ray vb.) araçların bitiş kodları vardıysa onları da buranın altına ekleyebilirsin.
   ...
   animate: function() {
        ...
        if (self.scene && self.renderer && self.camera) {
            ...
            if(self.currentMesh) {
                ...
                // --- İŞTE HATA VEREN SATIRIN GÜVENLİ HALİ ---
                // Eğer böyle bir fonksiyon varsa çalıştır, yoksa hata verme yola devam et!
                if (typeof self.updateHandlePositions === 'function') {
                    if (typeof self.updateHandlePositions === 'function') { self.updateHandlePositions(); }
                }
                // ---------------------------------------------
                
                // Güvenli Prizma Etiketleri
...
// DÖNDÜRME SIFIRLAMA
    ...
    // 2. Taşıma İşlemini Bitir (ARTIK HEM MOVE HEM SNAPSHOT MODUNDA ÇALIŞMALI)
    if ((currentTool === 'move' || currentTool === 'snapshot') && isMoving) {
        ...
        // EĞER SADECE RESMİ TAŞIYIP BIRAKTIYSAK, YENİ BİR KOPYALAMA ALANI AÇMASINI ENGELLE
        if (currentTool === 'snapshot') return; 
    }
        ...
        if (w > 10 && h > 10) {
            ...
            if (x >= 0 && y >= 0 && x + w <= canvas.width && y + h <= canvas.height) {
                ...
                newImg.onload = () => {
                    ...
                    // ▼▼▼ DEĞİŞTİRİLECEK KISIM BURASI ▼▼▼
                    // currentTool = 'move';       <-- Başına // koyup iptal et
                    // setActiveTool('move');      <-- Başına // koyup iptal et
                    
                    selectedItem = newObj;         // Resmi seçili bırak (butonlar görünsün)
                    isMoving = false; 
                    // returnToSnapshot = true;    <-- Başına // koyup iptal et
                    // ▲▲▲ ▲▲▲ ▲▲▲ ▲▲▲ ▲▲▲ ▲▲▲ ▲▲▲
...
if (window.Scene3D) {
    window.Scene3D.createUnfoldablePrism = function(sides, size, height, type) {
        ...
        const isCone = typeStr.includes('cone') || typeStr.includes('koni');
        const isPyramid = (typeStr.startsWith('pyramid') || typeStr.includes('cone') || typeStr.includes('koni')) && !isCone; 
        const isSphere = typeStr.includes('sphere') || typeStr.includes('küre');
        ...
        // 3) KÜRE
            ...
            for (let i = 0; i < n; i++) {
                ...
                for (let r = 0; r <= rows; r++) {
                    ...
                    for (let c = 0; c <= cols; c++) {
                        const u = (c / cols) - 0.5, phi = u * (Math.PI * 2 / n);
                        const lx0 = R * Math.sin(theta) * Math.sin(phi), ly0 = R - R * Math.cos(theta), lz0 = R * Math.sin(theta) * Math.cos(phi);
                        target0[idx] = lx0 * cosA + lz0 * sinA; target0[idx+1] = ly0; target0[idx+2] = -lx0 * sinA + lz0 * cosA;
                        const lx1 = R * Math.sin(theta) * phi, ly1 = 0, lz1 = R * theta; 
                        target1[idx] = lx1 * cosA + lz1 * sinA; target1[idx+1] = ly1; target1[idx+2] = -lx1 * sinA + lz1 * cosA;
                        verts[idx] = target0[idx]; verts[idx+1] = target0[idx+1]; verts[idx+2] = target0[idx+2]; idx += 3;
                    }
                }
        ...
        // 4) PRİZMA VE SİLİNDİR
            ...
            const offsetGroup = new THREE.Group(); offsetGroup.position.set(-s/2, 0, -Ri); group.add(offsetGroup);

            let parentHinge = offsetGroup;
            for (let i = 0; i < n; i++) {
                const hinge = new THREE.Group(); if (i === 0) hinge.position.set(0, 0, 0); else hinge.position.set(s, 0, 0); parentHinge.add(hinge);
                const faceMesh = new THREE.Mesh(faceGeo, mat); if (n < 20) faceMesh.add(new THREE.LineSegments(new THREE.EdgesGeometry(faceGeo), lineMat)); hinge.add(faceMesh);
                if (i > 0) animParts.push({ mesh: hinge, axis: 'y', closedAngle: -foldAngle, openAngle: 0 });
                if (i === 0) {
                    const tH = new THREE.Group(); tH.position.set(0, height/2, 0); hinge.add(tH);
                    const tM = new THREE.Mesh(topGeo, mat); if (n < 20) tM.add(new THREE.LineSegments(new THREE.EdgesGeometry(topGeo), lineMat)); tH.add(tM); animParts.push({ mesh: tH, axis: 'x', closedAngle: Math.PI/2, openAngle: 0 });
                    const bH = new THREE.Group(); bH.position.set(0, -height/2, 0); hinge.add(bH);
                    const bM = new THREE.Mesh(botGeo, mat); if (n < 20) bM.add(new THREE.LineSegments(new THREE.EdgesGeometry(botGeo), lineMat)); bH.add(bM); animParts.push({ mesh: bH, axis: 'x', closedAngle: -Math.PI/2, openAngle: 0 });
                }
                parentHinge = hinge; 
            }
    ...
    window.updatePyramidGeometry = function(group, ratio) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            let gamma = (1 - ratio) * Math.atan2(H, -Ri), Ax = Mx + L * Math.cos(gamma) * (Mx / Ri), Ay = L * Math.sin(gamma), Az = Mz + L * Math.cos(gamma) * (Mz / Ri);
            const pos = faces[i].geometry.attributes.position.array;
            pos[0] = V1.x; pos[1] = V1.y; pos[2] = V1.z; pos[3] = V2.x; pos[4] = V2.y; pos[5] = V2.z; pos[6] = Ax; pos[7] = Ay; pos[8] = Az;   
            faces[i].geometry.attributes.position.needsUpdate = true; faces[i].geometry.computeVertexNormals();
            if (faces[i].userData.edges) { faces[i].userData.edges.geometry.dispose(); faces[i].userData.edges.geometry = new THREE.EdgesGeometry(faces[i].geometry); }
        }
    };

    window.updateSphereGeometry = function(group, ratio) {
        const slices = group.userData.slices;
        for (let i = 0; i < slices.length; i++) {
            const mesh = slices[i], pos = mesh.geometry.attributes.position.array, t0 = mesh.userData.target0, t1 = mesh.userData.target1;
            for (let j = 0; j < pos.length; j++) pos[j] = t0[j] + (t1[j] - t0[j]) * ratio;
            mesh.geometry.attributes.position.needsUpdate = true; mesh.geometry.computeVertexNormals();
            if (mesh.children.length > 0) { mesh.children[0].geometry.dispose(); mesh.children[0].geometry = new THREE.EdgesGeometry(mesh.geometry, 15); }
        }
    };
    ...
    window.Scene3D.onUp = function() {
        ...
        if (wasPreviewing && this.previewMesh) {
            ...
            if (isUnfoldable && typeof this.createUnfoldablePrism === 'function') {
                ...
                solidShape = new THREE.Mesh(geometry, mat);
                solidShape.add(new THREE.LineSegments(new THREE.EdgesGeometry(geometry), new THREE.LineBasicMaterial({color: 0xffffff})));
    ...
    if (!window.Scene3D._meshTrapInstalled) {
        ...
        Object.defineProperty(window.Scene3D, 'currentMesh', {
            get: function() { return actualMesh; },
            set: function(val) {
                if (val) {
                    let obj = val;
                    while (obj && obj.type !== 'Scene') {
                        if (obj.userData && (obj.userData.isMathPyramid || obj.userData.isMathCone || obj.userData.isMathSphere || obj.userData.animParts)) {
                            actualMesh = obj; return;
                        }
                        obj = obj.parent;
                    }
                }
...
document.addEventListener('input', function(e) {
    if (e.target && e.target.id === 'unfold-slider') {
        ...
        if (mesh && !mesh.userData.animParts && !mesh.userData.isMathPyramid && !mesh.userData.isMathCone && !mesh.userData.isMathSphere) {
            let obj = mesh;
            while (obj && obj.type !== 'Scene') {
                if (obj.userData && (obj.userData.animParts || obj.userData.isMathPyramid || obj.userData.isMathCone || obj.userData.isMathSphere)) {
                    mesh = obj; break;
                }
                obj = obj.parent;
            }
            ...
            if (mesh.userData.isMathPyramid && typeof window.updatePyramidGeometry === 'function') window.updatePyramidGeometry(mesh, ratio);
                ...
                mesh.userData.animParts.forEach(part => {
                    if (part.mesh && part.closedAngle !== undefined) {
                        let openA = part.openAngle !== undefined ? part.openAngle : 0;
                        part.mesh.rotation[part.axis] = part.closedAngle + (openA - part.closedAngle) * ratio;
                    }
                });
            }
